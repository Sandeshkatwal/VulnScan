...
Version 14.8 adds a fix-first dashboard for prioritised findings. It is generated by default when `--prioritise` is used, or explicitly with `--fix-first-dashboard`:

```powershell
.\.venv311\Scripts\python.exe -m scanner.main scan --target 127.0.0.1 --prioritise --fix-first-dashboard
.\.venv311\Scripts\python.exe -m scanner.main scan --target 127.0.0.1 --vuln-intel --use-cve-feed --use-epss --use-exploit-metadata --prioritise --fix-first-dashboard --cve-feed data\cve_feeds\sample_cve_feed.json --epss-file data\epss\sample_epss.csv --exploit-metadata-file data\exploit_metadata\sample_exploit_metadata.json --json --html --save-db
```

The dashboard includes `fix_first_dashboard`, `priority_distribution`, `top_fix_first_findings`, `remediation_action_plan`, and `executive_summary` in JSON and HTML. It uses prioritised findings only and does not perform new scanning, exploit checks, live attack checks, or internet feed fetching. SLA hints are generic and should be customised. Human validation is still required.

Saved finding exports include dashboard fields where available: `priority_score`, `priority_label`, `recommended_action`, `sla_hint`, and `fix_first_rank`. Version 14.8 also adds:
