...
Version 12.3 adds opt-in read-only Windows Firewall and Microsoft Defender status collection on top of the Windows SMB/WinRM audit foundation, WinRM authentication validation, and host information collection. Foundation checks still use safe TCP socket reachability only for SMB `445`, NetBIOS/SMB `139`, WinRM HTTP `5985`, WinRM HTTPS `5986`, and RDP `3389`.
...
Allowed `--windows-auth-method` values are `none`, `smb`, and `winrm`; the default is `none`. Version 12.3 implements authentication validation only for `winrm`. `smb` remains a planned/foundation mode. WinRM authentication requires both `--windows-user` and `--windows-password`; VulScan does not prompt interactively and does not store or print the password. `--windows-host-info` and `--windows-security-status` also require `--windows-audit`, `--windows-auth-method winrm`, `--windows-user`, and `--windows-password`.
...
Windows audit results include `windows_audit_summary`, optional `windows_host_info`, optional `windows_security_status`, standard findings with source `windows_audit` or `windows_security_audit`, and a normalised `credentialed_audits` entry. Version 12.3 does not exploit, brute force, enumerate shares, dump credentials, modify systems, restart services, or validate vulnerabilities. WinRM should be restricted to trusted networks. Deeper authenticated Windows policy and patch checks are planned for later versions.
