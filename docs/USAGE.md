...
Version 12.2 adds opt-in read-only Windows host information collection on top of the Windows SMB/WinRM audit foundation and WinRM authentication validation. Foundation checks still use safe TCP socket reachability only for SMB `445`, NetBIOS/SMB `139`, WinRM HTTP `5985`, WinRM HTTPS `5986`, and RDP `3389`.
...
Allowed `--windows-auth-method` values are `none`, `smb`, and `winrm`; the default is `none`. Version 12.2 implements authentication validation only for `winrm`. `smb` remains a planned/foundation mode. WinRM authentication requires both `--windows-user` and `--windows-password`; VulScan does not prompt interactively and does not store or print the password. `--windows-host-info` also requires `--windows-audit`, `--windows-auth-method winrm`, `--windows-user`, and `--windows-password`.
...
Windows audit results include `windows_audit_summary`, optional `windows_host_info`, standard findings with source `windows_audit`, and a normalised `credentialed_audits` entry. Version 12.2 does not exploit, brute force, enumerate shares, dump credentials, modify systems, restart services, or validate vulnerabilities. WinRM should be restricted to trusted networks. Deeper authenticated Windows policy and patch checks are planned for later versions.
