...
Version 12.1 adds a safe WinRM authentication validation on top of the Windows SMB/WinRM audit foundation. Foundation checks still use safe TCP socket reachability only for SMB `445`, NetBIOS/SMB `139`, WinRM HTTP `5985`, WinRM HTTPS `5986`, and RDP `3389`.
...
Allowed `--windows-auth-method` values are `none`, `smb`, and `winrm`; the default is `none`. Version 12.1 implements authentication validation only for `winrm`. `smb` remains a planned/foundation mode. WinRM authentication requires both `--windows-user` and `--windows-password`; VulScan does not prompt interactively and does not store or print the password.

WinRM endpoint selection uses the reachability results: HTTPS `5986` is preferred, then HTTP `5985`. VulScan uses `pywinrm` if it is installed. If `pywinrm` is missing, the scan records `WINRM_DEPENDENCY_MISSING` and continues without a crash. The validation runs one harmless read-only command such as `hostname`; it does not run registry queries, patch enumeration, local user enumeration, process listing, file listing, credential dumping, privilege checks, or system modification commands. For lab HTTPS endpoints with self-signed certificates, certificate validation may be relaxed and this is recorded as a limitation.

Windows audit results include `windows_audit_summary`, standard findings with source `windows_audit`, and a normalised `credentialed_audits` entry. Version 12.1 does not exploit, brute force, enumerate shares, dump credentials, modify systems, restart services, or validate vulnerabilities. WinRM should be restricted to trusted networks. Deeper authenticated Windows policy and patch checks are planned for later versions.
