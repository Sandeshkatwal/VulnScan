...
Version 12.8 keeps Windows audit evidence concise by default. Findings do not store full raw PowerShell output; they store short evidence summaries and, where useful, safe observed/expected values for remediation and verification. Passwords are not stored or printed, and evidence/report/export paths redact password, token, authorization header, private key, hash, and credential-like patterns. Users should still avoid putting real credentials in command history where possible.

Version 12.9 adds timeout controls for slower WinRM environments: `--windows-timeout 10`, `--windows-command-timeout 15`, and `--windows-audit-timeout 120` are the defaults. Increase them only when authorised targets are slow or remote; lower them for fast lab checks. If a command or section times out, VulScan marks the Windows audit partial, keeps completed findings and summaries, records performance fields, and skips remaining work when the overall budget is exhausted. Use `--no-windows-progress` to suppress compact terminal progress messages.

Version 12.10 adds normalised Windows audit section results without changing user-facing commands. Reports now include `windows_audit_sections` when `--windows-audit` is used, while keeping `windows_audit_summary`, `windows_audit_consolidated_summary`, `credentialed_audits`, and the standard `findings` list. Normalised sections avoid secrets and raw command output by default and make future Windows profiles, dashboards, and API responses easier to build.
