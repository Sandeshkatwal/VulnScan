...
Version 14.4 adds a local vulnerability intelligence foundation with version-aware rules, optional local CVE-style feed import, and offline EPSS metadata enrichment. It normalises discovered services/software into `software_inventory`, evaluates local JSON rules from `data\vuln_intel\sample_vuln_rules.json` by default, can evaluate local feed records from `data\cve_feeds\sample_cve_feed.json`, can enrich matched CVEs from `data\epss\sample_epss.csv`, and emits `vuln_intel`, `cve_feed`, or importer status findings through the standard findings pipeline. It does not fetch live CVE, EPSS, Exploit-DB, or Metasploit data, and intelligence matches are indicators that require validation. Version-specific rules and feed records require local product/version evidence unless explicitly marked as unknown-version indicators. EPSS is a prioritisation signal, not proof of exploitation. See `docs\VULNERABILITY_INTELLIGENCE.md`.
...
Version 14.4 adds local vulnerability intelligence matching with `--vuln-intel`, local CVE-style feed matching with `--use-cve-feed`, and local EPSS metadata enrichment with `--use-epss`. Rules, feeds, and EPSS metadata are loaded only from local files, matched against normalised service/software inventory, and reported as conservative indicators. Version-aware rules and feed affected-version ranges require product/version evidence; unknown versions are treated as insufficient evidence. Version 14.4 does not download CVE feeds, EPSS data, exploit metadata, exploit code, or perform live vulnerability checks.
