...
Version 12.5 also includes Windows SMB/WinRM audit foundation checks, optional single-attempt WinRM authentication validation, opt-in read-only Windows host information collection, opt-in Windows Firewall and Microsoft Defender status collection, and opt-in local security policy indicators from `net accounts` using explicitly provided credentials.
...
Version 12.5 Windows audit checks TCP reachability for SMB, NetBIOS/SMB, WinRM HTTP, WinRM HTTPS, and RDP. With `--windows-auth-method winrm`, it requires `--windows-user` and `--windows-password`, prefers HTTPS on 5986 over HTTP on 5985, uses `pywinrm` when installed, and performs one safe read-only validation command. With `--windows-policy-status`, it runs only `net accounts` after successful WinRM authentication and parses password and lockout policy indicators. It does not modify password or lockout policy, export security policy, perform full Group Policy analysis, query registry, enumerate users, groups, files, shares, patches, secrets, hashes, tokens, or private keys, exploit, brute force, modify systems, or restart services. Domain policy may override local indicators. Passwords are not stored or printed.
