...
Version 16.2 adds a local React + Vite vulnerability list and finding detail UI on top of the local FastAPI API. The dashboard is local development only, runs on `http://localhost:5173`, and displays API health, safe scan job creation, recent jobs, selected job details, result summaries, recent scans, a read-only vulnerability list, and finding details. Version 16.3 adds a Risk Overview for the selected completed job, using loaded findings and job result data to summarise severity, priority, sources, top risks, asset criticality, and available trend context. Version 16.4 adds a Trends View for prioritisation trend data from completed jobs. Trends require scans run with `--priority-trends` and `--save-db`; the first scan becomes a baseline, and second or later scans can show improved, worsened, stable, or baseline comparisons. Trend matching uses stable finding keys and may still need human review. The dashboard remains local, read-only, and includes no exploit or credential controls. Start the backend first:
